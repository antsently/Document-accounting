...
Public Class Form17
    ...
    Sub Editor_zap()
        Try
            Dim Command As New OleDbCommand("UPDATE [Располяжения] SET [Регистрационный номер]='" & p1 & "', [Дата регистрации]='" & p2 & "', [Краткое содержание]='" & p3 & "', [Количество листов]='" & p4 & "', [Исполнитель]='" & p5 & "', [Рассылка]='" & p6 & "', [Отметка о помещении в дело]='" & p7 & "' WHERE ([Регистрационный номер] Like '" & ListView1.SelectedItems.Item(0).Text & "')", Connector)
            Connector.Open()
            Command.ExecuteNonQuery()
            Connector.Close()
    ...
    Sub Удалить()
        ...
        Try
            ...
            If МВох = DialogResult.Yes Then
                Dim Command As New OleDbCommand("DELETE FROM [Распряжения] WHERE [Регистрационный номер] '" & ListView1.SelectedItems.Item(0).Text & "'", Connector)
                Connector.Open()
                Command.ExecuteNonQuery()
                Connector.Close()
    ...
    Private Sub УбратьКолонкуToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles УбратьКолонкуToolStripMenuItem.Click
        ...
        Try
            ...
            OtchetGurnal = "Включение фильтра колонок Распоряжения " & DateString & " " & TimeString : ZapGurnal()
        Catch ex As Exception
    ...
    Private Sub ПоискПоНомеруToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ПоискПоНомеруToolStripMenuItem.Click
        ...
        Try
            ...
            For Each columnheader In Me.ListView1.Columns
                columnheader.Width = -3
            Next
            OtchetGurnal = "Выключение фильтра по номеру Распоряжения " & DateString & " " & TimeString : ZapGurnal()
        Catch ex As Exception
            MessageBox.Show("Cлeдyeт вводить только числа", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error)
            OtchetGurnal = "Ошибка" & ex.Message & DateString & " " & TimeString : ZapGurnal()
        End Try
    ...
    Private Sub ЭкспортExcelToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ЭкспортExcelToolStripMenuItem.Click
        Try
            ...
            Dim Y As Integer
            Dim z As Integer
            OtchetGurnal = "Запущен Экспорт Excel Распоряжения " & DateString & " " & TimeString : ZapGurnal()
    ...
    Private Sub ПоискПоДатеToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ПоискПоДатеToolStripMenuItem.Click
        ...
        Try
            ...
            For Each columnheader In Me.ListView1.Columns
                columnheader.Width = -3
            Next
            OtchetGurnal = "Включен фильтр по дате Распоряжения " & DateString & " " & TimeString : ZapGurnal()
        Catch ex As Exception
            MessageBox.Show("Cлeдyeт вводить только числа", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error)
            OtchetGurnal = "Ошибка" & ex.Message & DateString & " " & TimeString : ZapGurnal()
        End Try
    ...
    Private Sub ToolStripTextBox1_KeyDown(sender As Object, e As KeyEventArgs) Handles ToolStripTextBox1.KeyDown
        If e.KeyCode = Keys.Enter Then
            ...
            Else
                Form17_Load(Me, New EventArgs)
                OtchetGurnal = "Включение фильтра Распоряжение " & DateString & " " & TimeString : ZapGurnal()
            End If
        End If
    End Sub
    ...
    Private Sub ToolStripButton3_Click(sender As Object, e As EventArgs) Handles ToolStripButton3.Click
        ...
        OtchetGurnal = "Выключение фильтра записи Распоряжение " & DateString & " " & TimeString : ZapGurnal()
        Form17_Load(Me, New EventArgs)
    End Sub
